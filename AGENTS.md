...
3. Add the package to the relevant `home.packages` or `environment.systemPackages` list.
4. Use `nurl` to get the source hash.

Example structure for livekit-cli:
```
pkgs/clis/livekit-cli.nix           # Package definition
users/jossephus/default.nix         # Installs it for the user
```
